...
class NetworkAppRepository(
    ...
    override suspend fun publishAdminKnowledgeNode(id: String): AdminNodeItem = apiCall { api.publishAdminKnowledgeNode(id).let { AdminNodeItem(it.id,it.name,it.subject,it.grade,it.chapter,it.definition,it.explanation,it.reviewStatus,it.reviewStatus == "approved",it.version,it.sourceExcerpt) } }
    override suspend fun withdrawAdminKnowledgeNode(id: String): AdminNodeItem = apiCall { api.withdrawAdminKnowledgeNode(id).let { AdminNodeItem(it.id,it.name,it.subject,it.grade,it.chapter,it.definition,it.explanation,it.reviewStatus,it.reviewStatus == "approved",it.version,it.sourceExcerpt) } }
